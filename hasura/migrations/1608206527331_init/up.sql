CREATE FUNCTION public.check_content_length() RETURNS trigger
    LANGUAGE plpgsql
    AS $_$
DECLARE
    ...
    DECLARE password_length INTEGER;
...
BEGIN
    ...
    IF NEW.password IS NOT NULL THEN
        select length(NEW.password) INTO password_length;
        IF email_length < 5 THEN
            RAISE EXCEPTION 'Password can not have less than 5 characters';
        END IF;
        IF password_length > 50 THEN
            RAISE EXCEPTION 'Password can not have more than 50 characters';
        END IF;
    END IF;
...
ALTER TABLE ONLY public.user_sessions
    ADD CONSTRAINT user_sessions_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id) ON UPDATE RESTRICT ON DELETE RESTRICT;
